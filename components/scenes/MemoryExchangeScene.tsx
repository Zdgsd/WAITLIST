...
const MemoryExchangeSceneComponent: React.FC<MemoryExchangeSceneProps> = ({ email, memoryNumber, onSubmit, onExit, triggerBackgroundAnimation }) => {
  ...
  const renderConversation = () => {
    return (
      <div className={`animate-fade-in text-xl md:text-2xl lg:text-3xl space-y-6 transition-opacity duration-300 ${isClearing ? 'opacity-0' : 'opacity-100'}`}>
        {conversationStep === 'name' && (
          <form onSubmit={(e) => { e.preventDefault(); if(memoryCard.name) handleConversationSubmit('age'); }}>
            ...
            <input id="name-input" type="text" value={memoryCard.name} onChange={e => setMemoryCard(p => ({...p, name: e.target.value}))} autoFocus className="w-full bg-transparent focus:outline-none border-b-2 border-[var(--terminal-green)]/50 focus:border-[var(--terminal-green)] transition-colors text-2xl md:text-3xl"/>
  ...
  const renderGame = () => {
    ...
    return (
        <div className="animate-fade-in text-xl md:text-2xl">
            ...
            <p className="mb-6 min-h-[4rem] flex items-center">{currentQuestion.title}</p>
            ...
            <div className="space-y-6">
             ...
             <div className="mt-8 flex items-center justify-between">
                ...
                <Button onClick={() => { 
                    ...
                    onSubmit(memoryCard);
                }} disabled={!memoryCard.consentGiven}>Join the community</Button>
            </div>
        </div>
    );
  };
