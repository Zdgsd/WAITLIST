      ...
      <div className={`animate-fade-in text-xl md:text-2xl lg:text-3xl space-y-6 transition-opacity duration-300 ${isClearing ? 'opacity-0' : 'opacity-100'}`} aria-live="polite">
        ...
        {conversationStep === 'age' && (
          <form onSubmit={(e) => { e.preventDefault(); if(memoryCard.age) { onAgeSubmitted(); handleConversationSubmit('post_age_transition'); } }} role="form" aria-label="Enter your age">
            ...
            <input id="age-input" type="number" value={memoryCard.age || ''} onChange={e => setMemoryCard(p => ({...p, age: parseInt(e.target.value) || null}))} autoFocus className="w-full bg-transparent focus:outline-none2 border-b-2 border-[var(--terminal-green)]/50 focus:border-[var(--terminal-green)] transition-colors text-2xl md:text-3xl" aria-required="true" aria-describedby="age-input-description" min="1" max="120"/>
