...
// Group scene components by route/phase for better code splitting
const IntroScenes = {
  CorporateShell: React.lazy(() => import('./components/scenes/CorporateShell').then(module => ({ default: module.CorporateShell }))),
  InitializationScene: React.lazy(() => import('./components/scenes/InitializationScene').then(module => ({ default: module.InitializationScene }))),
  TypewriterScene: React.lazy(() => import('./components/scenes/TypewriterScene').then(module => ({ default: module.TypewriterScene }))),
  GlitchScene: React.lazy(() => import('./components/scenes/GlitchScene').then(module => ({ default: module.GlitchScene })))
};

const MainScenes = {
  BrandRevealScene: React.lazy(() => import('./components/scenes/BrandRevealScene').then(module => ({ default: module.BrandRevealScene }))),
  InvitationScene: React.lazy(() => import('./components/scenes/InvitationScene').then(module => ({ default: module.InvitationScene }))),
  MemoryExchangeScene: React.lazy(() => import('./components/scenes/MemoryExchangeScene').then(module => ({ default: module.MemoryExchangeScene }))),
  CompletionScene: React.lazy(() => import('./components/scenes/CompletionScene').then(module => ({ default: module.CompletionScene }))),
  ExitScene: React.lazy(() => import('./components/scenes/ExitScene').then(module => ({ default: module.ExitScene })))
};

const CorporateScenes = {
  InvestorPage: React.lazy(() => import('./components/scenes/InvestorPage').then(module => ({ default: module.InvestorPage })))
};
...
import { useSocialMediaDetection } from './hooks/useSocialMediaDetection';
import { usePlatformOptimization } from './hooks/usePlatformOptimization';

const AppContent: React.FC = () => {
    useSocialMediaOptimization();
    const socialMediaContext = useSocialMediaDetection();
    ...
    const renderPhase = () => {
        switch (phase) {
            case AppPhase.CORPORATE_SHELL:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <IntroScenes.CorporateShell onComplete={() => {
            ...
            case AppPhase.INITIALIZATION:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <IntroScenes.InitializationScene onComplete={() => handlePhaseChange(AppPhase.IMAGINE_IF)} />
                    </Suspense>
                );
            case AppPhase.IMAGINE_IF:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <IntroScenes.TypewriterScene text="Imagine If..." onComplete={() => handlePhaseChange(AppPhase.GLITCH_1)} finalBlinkOnce />
                    </Suspense>
                );
            case AppPhase.GLITCH_1:
                 return (
                    <Suspense fallback={<SceneLoader />}>
                        <IntroScenes.GlitchScene onComplete={() => handlePhaseChange(AppPhase.MEMORY_PROMPT_1)} />
                    </Suspense>
                );
            case AppPhase.MEMORY_PROMPT_1:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <IntroScenes.TypewriterScene text="Your Best Memory..." onComplete={() => handlePhaseChange(AppPhase.MEMORY_PROMPT_2)} />
                    </Suspense>
                );
            case AppPhase.MEMORY_PROMPT_2:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <IntroScenes.TypewriterScene text="Is A Click Away." onComplete={() => handlePhaseChange(AppPhase.BRAND_REVEAL)} />
                    </Suspense>
                );
            case AppPhase.BRAND_REVEAL:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <MainScenes.BrandRevealScene onComplete={() => handlePhaseChange(AppPhase.INVITATION_BOX)} skipToEnd={isSkipping} />
                    </Suspense>
                );
            case AppPhase.INVITATION_BOX:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <MainScenes.InvitationScene onComplete={handleInvitationComplete} triggerBackgroundAnimation={triggerBackgroundAnimation} />
                    </Suspense>
                );
            case AppPhase.MEMORY_EXCHANGE:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <MainScenes.MemoryExchangeScene email={memoryData.email} memoryNumber={memoryNumber} onSubmit={handleMemorySubmit} onExit={() => handlePhaseChange(AppPhase.EXIT)} triggerBackgroundAnimation={triggerBackgroundAnimation} onAgeSubmitted={handleAgeSubmitted} />
                    </Suspense>
                );
            case AppPhase.COMPLETION:
                return memoryData && memoryNumber ? (
                    <Suspense fallback={<SceneLoader />}>
                        <MainScenes.CompletionScene 
                          data={memoryData} 
            ...
            case AppPhase.INVESTOR_PAGE:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <CorporateScenes.InvestorPage onBack={() => handlePhaseChange(AppPhase.COMPLETION)} />
                    </Suspense>
                );
            case AppPhase.EXIT:
                return (
                    <Suspense fallback={<SceneLoader />}>
                        <MainScenes.ExitScene />
                    </Suspense>
                );
            default:
                return (
    ...
    return (
      <main className="h-screen w-screen overflow-hidden p-2 md:p-4 relative">
        {!socialMediaContext.isSocialMedia && (
          <>
        ...
        <ErrorBoundary>
          ...
          >
            ...
            {!showChatModal && (
              <>
                <nav aria-label="Quick actions" className="fixed top-4 right-4 z-[1002] flex space-x-2">
                  <button
